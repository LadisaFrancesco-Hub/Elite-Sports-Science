...
**Redesign visivo Coach Console — pass mirato su gap reali (2026-09-24)**
- Contesto: brief "full visual redesign" (stile Linear/Vercel/WHOOP). Analisi codice → gran parte del brief era **già implementata** dal Design System v3 (topbar/sidebar/card/tabelle/input già scuri, palette token già ~identica al target: `--s3 #1C2128` e `--blue #3B82F6` combaciano esatti, `--teal` = arancione oklch hue 52). La "current state" del brief era in gran parte obsoleta. Confermato con l'utente lo scope: 4 gap reali visibili negli screenshot.
- **Problema #1 — chrome arancione (il #1 del brief)**: la barra arancione in cima a ogni screenshot NON era la topbar dell'app (già scura) ma il **chrome browser/PWA** pilotato da `theme-color`. Fix: `index.html` `<meta theme-color>` `#f97316`→`#14181E` (bordo alto del gradiente della topbar → blend seamless); `manifest.json` `theme_color` idem + `background_color` `#0e0a06` (marrone caldo)→`#0A0C0F` (neutro). Login/splash restano arancioni (logo + CTA = uso corretto dell'accento per il brief).
- **Gap #5 — avatar atleti**: `renderAthletes` (app.js) sovrascriveva inline l'avatar con una palette random (arancio/rosa/teal/viola). Rimossa la palette `_ATH_PALETTE`+`_athColor` (dead code) → l'avatar usa il default CSS `.ac-av` uniforme (`#1B2027`, bordo tenue, `--mono-dim`). Stato reso semantico (verde OK / coral ALTO) invece che tutto arancione. Hover card → bordo arancione tenue (`oklch(0.76 0.16 52 / .40)`).
- **Gap #6 — chip Storico**: sessione + fase erano tutti arancioni (`.tag.tg`). Nuova classe neutra `.tn` (`rgba(255,255,255,.06)` bg, bordo `.14`, testo `--text2`). Sostituiti i 4 chip sessione/fase (Storico tabella + card sessione + card storico mobile) `tg`→`tn`; il chip **flag** resta coral (`.tc`, critico). Aggiunto `escHtml` mancante su session/phase in tabella.
- **Gap #3 — palette grafici Analytics**: spark card `analytics.js` — Volume `--teal`(arancio)→`--blue`, e1RM `--amber`(giallo)→`--teal`(arancio, outcome hero); Carico resta `--purple`(viola). Ora sistema coerente Volume=blu / Carico=viola / E1RM=arancio.
- **Gap #8 — blocchi Macro Periodizzazione**: **bug reale** — il fill usava `${c}22` (trucco hex-alpha) ma `c` era un `var(--…)` → `var(--teal)22` = CSS invalido → blocchi senza riempimento (solo bordo), e fasi non definite quasi invisibili (aspetto "wireframe"). Fix: mappa `_MACRO_FILL` con rgba espliciti; ramp fasi ricolorata (Accumulo blu → Intensif arancio → Picco rosso → Scarico verde); blocchi più alti (min-height 54px), W# prominente dentro il blocco + sigla fase, hover brightness. Allineate le altre 2 mappe fase (meso card `app.js`, timeline meso) + legenda statica (`index.html`) allo stesso ramp per coerenza.
- Verifica: `node --check` OK (app.js/analytics.js), `manifest.json` valido, 0 ref rotte (`_athColor`/`${c}22`/`col.bg`), 0 `tag tg` residui, template Macro con tag bilanciati. **Smoke Playwright** (Chrome reale via `playwright-core` + `channel:'chrome'`, server statico locale, SW bloccato, modalità coach forzata via window bridge, seed demo): avatar 4/4 senza bg inline (uniformi); Storico 244 chip `.tn` / 0 `.tg` (flag resta coral); Macro 12/12 blocchi con fill `rgba()` reale / 0 fill `undefined` (bug risolto) + blocchi colorati per fase; spark colors = `[--blue, --purple, --teal]` (Volume blu/Carico viola/E1RM arancio). **0 errori console.** Screenshot in `/tmp/ess_shots/` (atleti/storico/macro/analytics confermati a occhio).
- Deploy **v6.81** su Vercel (`vercel --prod`, SW bumpato) — verificato live su https://coach-os-lime.vercel.app (SW v6.81, `theme-color`/`theme_color` = `#14181E` serviti in prod) ✅

**Redesign visivo App Atleta v4 — un solo accento + semantica rigida (2026-09-25)**
- Brief: redesign "premium" dei 6 pannelli atleta (Oggi, Settimana, Wellness, Sessione, Progressi, Coach) senza spostare bottoni né rompere funzioni.
- **Diagnosi**: il caos cromatico NON stava in `styles.css` ma negli stili inline dei template JS (≈100 colori hard-coded: verde menta `#00E5A8`, giallo `#FFB800`, viola `#A78BFA`, superfici bluastre `#0F1520/#1A2235`). Causa radice semantica: `--teal` (arancione, hue 52) significava sia "brand/azione" sia "buono", mentre `--amber` (hue 88, quasi adiacente) significava "attenzione" → due tinte vicine con significati opposti.
- **Regola v4**: ARANCIONE = azione + "i tuoi dati" (CTA, progresso, completato, PR, oggi, selezione) · VERDE/AMBRA/CORALLO = SOLO stato (ok/attenzione/rischio) · tutto il resto neutro · viola e blu eliminati dall'app atleta · un solo CTA pieno per schermata.
- **Scope**: `body.is-athlete` (aggiunta in `main.js` al login atleta) → la Coach Console non cambia (verificato: nessuna classe, radius card 9px invariato, 5 pannelli coach renderizzano).
- **styles.css**: nuovi token in `:root` (`--accent*`, `--ok/--warn/--bad` + wash, elevazione `--e1/e2/e3`, `--line`, `--sheen`, `--sh-card/float`, `--r-card`, `--ease-out`) + blocco "APP ATLETA v4" (§1–11: superfici, KPI, bottoni `ax-cta/ax-ghost`, pannelli, bottom bar con indicatore attivo, ingresso a cascata). Rispetta `prefers-reduced-motion`.
- **Template JS** (solo presentazione, logica invariata): `app.js` (home, settimana, week widget, risposte coach, chat), `workout.js` (card esercizio, fasi, timer, pallini log/PR come classi `is-log/is-pr`), `analytics.js` (Progressi + grafici e1RM), `wellness.js` (ring/etichette semantiche verdi), `nutrition.js`, `badges.js`, `index.html` (KPI live, CTA, mappa corpo, chat).
- **Grafici**: tema condiviso in `utils.js` (`AX`, `axChartOptions`, `axAreaFill`, `axLastPointOnly`, `axHonestRange`): gradiente sull'area reale, solo ultimo punto evidenziato, tooltip scuro mono, range Y minimo ±4% (147→150 kg non sembra più un salto).
- **Micro-interazioni**: `playEntrance()` in `utils.js`, chiamata da `go()`: count-up KPI (esclusi i KPI live della sessione, riletti da `endWorkout`) + sweep del ring readiness; sheen sui CTA d'ingresso; ping sul promemoria check-in; glow sul PR.
- **Bug corretti trovati durante il lavoro**: (1) toast "nascosto" solo traslato → su mobile restava una **pillola arancione vuota sopra la bottom bar** in ogni schermata (ora `visibility/opacity`); (2) KPI home "Volume (ult. 8)" mostrava **lo streak**, non il volume (ora media t/seduta); (3) grafico e1RM Progressi di default univa il massimo di **esercizi diversi** per seduta → finti crolli (ora default = esercizio più tracciato, "Tutti" resta selezionabile, nota sotto il grafico); (4) "0/0 esercizi" al primo render del set tracker; (5) grafico nutrizione con assi vuoti "0–1 kcal" senza dati (ora nascosto); (6) testo bianco su arancione 2.25:1 (fallisce WCAG) → inchiostro scuro 8.5:1.
- Verifica: `node --check` OK su 8 file; harness Playwright (Chromium, 390px, dati demo reali, atleta a2) con catture prima/dopo dei 6 pannelli; test interazioni: tap pallino → gradiente accento, realLog → `is-log`, PR reale → `is-pr` + badge, deselect pulisce le classi, timer REST, count-up (13 → 32 finale), ring verde, toast hidden/visible. 0 nuovi errori console. SW bumpato a v6.82.
- Merge fast-forward su `main` (`3d6d3ad`) + **deploy v6.82 in produzione** via API Vercel da gitSource (`dpl_F8ZMiPKaA1YhgvQbjfK56rWfoXTY`, READY, alias `coach-os-lime.vercel.app`) — verificato live: `sw.js` v6.82 + `is-athlete` in `main.js` serviti. Nota: il progetto Vercel NON è collegato al repo GitHub → il push su `main` non fa auto-deploy. Canvas prima/dopo pubblicato come Design artifact.
- Resta debole: grafico nutrizione a doppio asse (kcal vs proteine) — anti-pattern data-viz, da valutare se separarlo in due mini-grafici.

**Verifica indipendente redesign atleta v4 (2026-09-25)**
- Contesto: il redesign era stato prodotto da Claude Design ("Design in codebase"), che ha lavorato su GitHub — nuovo branch `claude/vibrant-maxwell-f20cqw` poi merge in `main` (`3d6d3ad`+`e4364a5`), NON sulla working copy locale. `main` locale allineato con `git pull --ff-only` (era fermo a `59c93d2`).
- **Review statica** dei moduli condivisi coach+atleta (il redesign tocca `app.js`, `analytics.js`, `workout.js`, `nutrition.js`, `wellness.js`, `badges.js`, `utils.js`, `index.html`, `styles.css`): `node --check --input-type=module` OK su tutti i 15 moduli JS. Confermato che: (1) `playEntrance()` chiamata da `go()` ha come prima riga il guard `if (!panel || !is-athlete) return` → **no-op reale sulla nav coach**; (2) i nuovi token (`--accent*`, `--ok/--warn/--bad`, `--sheen`, `--glow-accent`) sono tutti definiti in `:root` con `--ok/--warn/--bad` che aliasano green/amber/coral; (3) gli helper Chart.js (`AX`, `axAreaFill`, `axChartOptions`, `axLastPointOnly`, `axHonestRange`) usati da `renderE1rmChart` (grafico condiviso col coach) sono tutti esportati da `utils.js`; (4) scope `is-athlete` presente 66 volte in `styles.css`.
- **Smoke test Playwright** (Chrome reale via `playwright-core` + `channel:'chrome'`, SW bloccato, Supabase abortito via route, seed demo innescato a mano con `window.seed()`, atleta `a1` Niccolò):
  - **Atleta 390px** — catturate tutte le 6 tab: Oggi (banner check-in raffinato, nome arancio, CTA sheen, badge PR), Settimana (compliance/progress arancio), Wellness (ring readiness verde=ottimale, semantico), Sessione ("0 kg" arancio non più verde menta, RIR neutro, chip neutri), Progressi (KPI coerenti Volume arancio/ACWR verde/resto neutro + 2 canvas grafici), Coach. Caos cromatico originale risolto. **0 errori console**.
  - **Coach 1360px** — Coach Console **intatta**: dashboard con cruscotto settimanale (Elena CRITICO ACWR 1.62/rischio 160/infortunio, Giulia CRITICO, Niccolò/Marco OK), KPI arancioni, analisi automatica; tutti i pannelli renderizzano (dashboard/editor/analytics/storico/calendario/macro/messaggi). **0 errori console**.
- **Conclusione: redesign promosso.** Coach non rotto, atleta drasticamente migliorato, già live in produzione (SW `v6.82` verificato su `coach-os-lime.vercel.app`). Due micro-ritocchi cosmetici hanno però sfiorato il coach (colore proteine nella card nutrizione coach `#60a5fa`→`var(--text2)` e restyle del grafico e1RM condiviso) — solo estetici, non regressivi. Screenshot in `/tmp/ess_smoke/`.
